...
def login():
    ...
    # Gerekli kodları buraya ekleyebilirsiniz
    with open("begenilen_postlar.txt", "r") as file:
        veriler = file.read().split('--------------------')
    for veri in veriler:
        veri = veri.strip()
        if veri:
            satirlar = veri.strip().split("\n")
            post_metni = satirlar[0].split(": ")[1]
            post_linki = satirlar[1].split(": ")[1]
            post_data = satirlar[2].split(": ")[1]
            tree.insert("", "end", values=(post_metni, post_linki, post_data))
...
window.geometry("700x600")  # Genişlik: 300 piksel, Yükseklik: 200 piksel
...
tree = ttk.Treeview(window, columns=("Post Metni", "Post Linki", "Post Data"))
tree.heading("Post Metni", text="Post Metni")
tree.heading("Post Linki", text="Post Linki")
tree.heading("Post Data", text="Post Data")
tree.pack(pady=10)

# Sütun genişliklerini ayarla
tree.column("Post Metni", width=200)
tree.column("Post Linki", width=200)
tree.column("Post Data", width=200)
